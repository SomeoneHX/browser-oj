...
<template>
  <div class="page-container"><BaseCard padding="md" class="page-meta-card"><div class="page-meta-row"><div class="page-meta-left"><h2><i class="fas fa-list" />题目列表</h2></div><div class="page-meta-right"><span class="problem-count">共 {{ problems.length }} 题</span></div></div></BaseCard>
    <BaseCard flush><table class="problem-table"><thead><tr><th class="col-id">#</th><th class="col-title">题目名称</th><th class="col-diff">难度</th><th class="col-rate">通过率</th><th class="col-status">我的状态</th></tr></thead><tbody>
      <tr v-for="problem in problems" :key="problem.id"><td class="col-id"><RouterLink :to="`/problem/${problem.id}`" class="problem-link">{{ problem.id }}</RouterLink></td><td class="col-title"><RouterLink :to="`/problem/${problem.id}`" class="problem-link">{{ problem.title }}</RouterLink></td><td class="col-diff"><span :class="['tag', difficultyMap[problem.difficulty] || '']">{{ problem.difficulty }}</span></td><td class="col-rate">{{ row(problem.id).passRateText }}</td><td class="col-status"><span v-if="row(problem.id).status" :class="['status-badge', row(problem.id).isAc ? 'status-ac' : 'status-wa']"><i :class="['fas', row(problem.id).isAc ? 'fa-check-circle' : 'fa-times-circle']" />{{ row(problem.id).isAc ? '通过' : '未通过' }}</span><span v-else class="status-none">-</span></td></tr>
