...
<template>
  ...
  <div v-else class="page-container"><div class="page-header"><h2><button type="button" class="header-back-link" aria-label="返回上一页" @click="goBack"><i class="fas fa-arrow-left" /></button>评测详情</h2><span class="detail-id-label">#{{ sub.id }}</span></div>
    <div class="submission-detail"><div class="detail-header"><h3><i :class="['fas', ...statusConfig(sub.status)[0].split(' ')]" :style="{ color: statusConfig(sub.status)[2] }" />{{ statusConfig(sub.status)[1] }}</h3><span class="detail-time">{{ formatTime(sub.timestamp) }}</span></div>
